...
import monday_direct_events_v3 as events

ROOT = Path(__file__).resolve().parents[1]
PREFIX = 'evaluation/model-comparison/household-direct-v3/'
SPEC = ROOT / PREFIX
OUT = ROOT / 'validation/model-comparison/household-direct-v3'
...
def run_one(provider, case, p, bound, packages, freeze_raw, authorisation):
    ...
    r = {'schema': 'okf-direct-attempt.v3', 'provider': provider, 'case_id': case, 'attempt': 'attempt-01',
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='okf-direct-v3-') as directory:
