#!/usr/bin/env python3
"""Build an isolated passage-boundary successor catalogue for source review."""
...
from passage_boundary_parser import segment_source, VERSION
from build_pdf_structure import DEFAULT_OUTPUT as STRUCTURE_OUTPUT, run as verify_pdf_structure

OUTPUT = 'evaluation/passage-boundary-candidate/structured-units'
...
def compile_units(root=ROOT):
    ...
    for path in ('scripts/build_passage_boundary_units.py','scripts/passage_boundary_parser.py','scripts/manual_references.py','scripts/manual_auxiliary_structure.py','scripts/manual_navigation_regions.py','scripts/structured_projection_output.py',
...
def main():
    ...
    parser.add_argument('--profile',choices=('passage-boundary-v1',),required=True)
