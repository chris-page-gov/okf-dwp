...
export function inspectPackage(evidence, question, descriptor, manifestBytes) {
  ...
  assert.equal(evidence.bundle.snapshot, manifest.bundle.snapshot);
  ...
  return { context_id: evidence.context_id, context_snapshot: evidence.bundle.snapshot, reader_snapshot: descriptor.snapshot,
    evidence_status: evidence.evidence_status, budget: evidence.budget,
...
async function main() {
  ...
  try {
    ...
    await graphControls.getByRole('button', { name: /^Relationships \(/ }).click();
