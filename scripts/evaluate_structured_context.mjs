...
assert.equal(protocol.metrics_helper?.path,'scripts/structured_context_metrics.mjs','Pin the independent metrics helper');
read(protocol.metrics_helper.path,protocol.metrics_helper);
const {retainedContextPath,measureLocationNavigation}=await import(pathToFileURL(resolve(ROOT,protocol.metrics_helper.path)));
const locationLedger=protocol.location_ledger?JSON.parse(read(protocol.location_ledger.path,protocol.location_ledger)):null;
const legacyRequirements=protocol.legacy_requirements?JSON.parse(read(protocol.legacy_requirements.source.path,protocol.legacy_requirements.source)).requirements:[];
...
const comparisonMode=protocol.comparison_mode||'same-authored-semantics';
assert(['same-authored-semantics','additive-source-and-semantic-increment'].includes(comparisonMode));
if(comparisonMode==='same-authored-semantics'){
 ...
 const candidateRecords=new Map(sources[1].base.records.map(r=>[r.id,r]));
 for(const record of sources[0].base.records)assert.deepEqual(candidateRecords.get(record.id),record,'Inherited concept/scope changed');
 const candidate=new Map(sources[1].base.requirements.map(r=>[r.id,r]));
 for(const requirement of sources[0].base.requirements)assert.deepEqual(candidate.get(requirement.id),requirement,'Inherited requirement changed');
 const preservation=protocol.legacy_requirements;
 assert(preservation?.source&&preservation?.count===40,'Bind the unchanged legacy requirements separately');
 ...
 const declared=groups.flatMap(g=>{assert(typeof g.id==='string'&&Array.isArray(g.requirement_ids));return g.requirement_ids;});
 assert.equal(new Set(declared).size,declared.length,'Requirement groups overlap');
 assert.deepEqual([...declared].sort(),[...candidate.keys()].sort(),'Every requirement needs an explicit evaluation plane');
}
const inheritedRequirementIds=new Set(sources[0].base.requirements.map(r=>r.id));
...
for(const test of cases)for(const run of sources)for(const max_bytes of protocol.budgets){
 ...
 const retainedPath=p=>retainedContextPath(pack,p,resolved);
 const locationNavigation=locationLedger?measureLocationNavigation({context:pack,ledger:locationLedger,resolvedConceptIds:resolved,
  requirements:run.base.requirements,legacyRequirements,canonicalJson}):null;
 const inheritedPaths=requirements.filter(r=>inheritedRequirementIds.has(r.id)).flatMap(r=>r.required_paths||[]);
 const requirementGroups=(protocol.requirement_groups||[]).map(group=>{
  const ids=new Set(group.requirement_ids),active=requirements.filter(r=>ids.has(r.id)),expected=active.flatMap(r=>r.required_paths||[]);
  return {id:group.id,active_requirements:active.map(r=>r.id),required_paths:expected.length,retained_required_paths:expected.filter(retainedPath).length};
 });
 ...
 rows.push({case_id:test.id,question:test.question,stage:run.stage,max_bytes,context_id:pack.context_id,sha256:sha(bytes),bytes:bytes.length,
  ...
  resolved_concepts:pack.resolved_concepts.map(r=>r.id),declared_resolved_concepts:[...resolved].sort(),active_requirements:requirements.map(r=>r.id),
  required_paths:paths.length,retained_required_paths:paths.filter(retainedPath).length,
  inherited_required_paths:inheritedPaths.length,retained_inherited_paths:inheritedPaths.filter(retainedPath).length,requirement_groups:requirementGroups,
  location_navigation:locationNavigation,
  ...
  fetched_files:pack.retrieval?.fetched_files,fetched_bytes:pack.retrieval?.fetched_bytes,decoded_bytes:pack.retrieval?.decoded_bytes,
...
// A successful long run binds its closing state as well as its initial reads.
for(const ref of inputs.values()){
 const path=resolve(ROOT,ref.path);assert.equal(realpathSync(path),path);const raw=readFileSync(path);
 assert.equal(raw.length,ref.bytes,'Input changed during evaluation: '+ref.path);assert.equal(sha(raw),ref.sha256,'Input changed during evaluation: '+ref.path);
}
for(const [file,hash] of Object.entries(approved.files))assert.equal(sha(readFileSync(resolve(engine,file))),hash,'Engine changed during evaluation');
assert.deepEqual(readFileSync(fileURLToPath(import.meta.url)),runnerBytes,'Runner changed during evaluation');
