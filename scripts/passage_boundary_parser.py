...
from manual_navigation_regions import navigation_regions, local_heading_attachment
...
BARE_APPENDIX_HEADING = re.compile(r"^A\s*p\s*p\s*e\s*n\s*d\s*i\s*x\s*$", re.I)
    ...
    for pos, item in enumerate(structure["paragraphs"]):
        ...
        if compatible:
            ...
            if local:
                ...
                if not between and (doc.get("kind") != "historical-amendment"
                                    or local_heading_attachment(structure["lines"], nearest["end"], item["start"])):
...
def separate_section_events(doc, structure):
    ...
    for line in structure["lines"]:
        if (APPENDIX_HEADING.fullmatch(line.stripped)
                or (doc.get("kind") == "historical-amendment"
                    and BARE_APPENDIX_HEADING.fullmatch(line.stripped))):
...
def segment_source(family, doc, pages, authored=()):
    ...
    navigation = navigation_regions(doc, structure["lines"], structure["source_bytes"], structure["paragraphs"])
    admitted = []
    for paragraph in structure["paragraphs"]:
        covering = next((r for r in navigation if r["start"] <= paragraph["start"] < r["end"]), None)
        if covering:
            structure["rejected_number_lines"].append({"start": paragraph["start"], "page": paragraph["page"],
                "label": paragraph["label"], "reason": "number-in-literal-navigation-or-amendment-region"})
        else:
            admitted.append(paragraph)
    structure["paragraphs"] = admitted
    ...
    special.extend(navigation)
    auxiliary = literal_auxiliary_regions(pages, family, doc.get("chapter"))
    special.extend(auxiliary)
    for item in structure["paragraphs"]:
        if any(item["boundary_start"] < region["end"] <= item["start"] for region in [*auxiliary, *navigation]):
