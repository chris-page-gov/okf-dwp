#!/usr/bin/env node
/** Verify one exact v3 publication. Read-only HTTPS, confined paths, no retries. */
...
export const LIMITS = Object.freeze({ concurrent_requests: 4, timeout_ms_per_request: 20000,
  timeout_ms_overall: 600000,
  max_bytes_per_response: 4194304, max_total_bytes: 67108864, max_manifest_bytes: 2097152,
...
export function validateManifest(manifest, commit) {
  check(/^[a-f0-9]{40}$/.test(commit) && manifest.schema === 'okf-dwp-learning-site.v3'
    && manifest.source_commit === commit, 'Wrong v3 source identity');
  ...
  const workbench = manifest.evidence_workbench;
  check(workbench && workbench.schema === 'okf-dwp-workbench-publication.v1'
    && workbench.questions === 40 && Number.isSafeInteger(workbench.files)
    && workbench.files >= 81 && workbench.files <= 4096
    && Number.isSafeInteger(workbench.bytes) && workbench.bytes > 0
    && workbench.bytes <= 56 * 1024 * 1024
    && /^[a-f0-9]{64}$/.test(workbench.manifest_sha256), 'Invalid public workbench receipt');
  ...
  const outputs = new Map(); let total = 0, workbenchBytes = 0, workbenchFiles = 0;
  for (const row of manifest.files) {
    ...
    } else if (row.path.startsWith('evaluation/evidence-workbench/')) {
      check(/^evaluation\/evidence-workbench\/(?:manifest\.json|packages\/staff-[0-9]{3}\.json|parts\/staff-[0-9]{3}-[0-9]{3}\.json)$/.test(row.path),
        'Unexpected workbench output');
      workbenchBytes += row.bytes; workbenchFiles++;
  ...
  check(workbenchFiles === workbench.files && workbenchBytes === workbench.bytes,
    'Workbench receipt and public file census differ');
  check(outputs.get('evaluation/evidence-workbench/manifest.json')?.sha256 === workbench.manifest_sha256,
    'Workbench manifest hash differs from receipt');
...
export function validateWorkbenchManifest(manifest, plan) {
  check(manifest?.schema === 'okf-evidence-workbench.v1'
    && manifest.source?.registry === 'evaluation/staff-questions/cases.json'
    && manifest.source?.corpus === 'structured-context/evidence-connect-manifest.json'
    && /^[a-f0-9]{64}$/.test(manifest.source.registry_sha256)
    && /^[a-f0-9]{64}$/.test(manifest.source.corpus_sha256), 'Invalid workbench source binding');
  check(Array.isArray(manifest.questions) && manifest.questions.length === 40, 'Invalid workbench case count');
  const expected = new Set(['evaluation/evidence-workbench/manifest.json']);
  const questions = new Map();
  const publicRef = (ref, path, cap) => {
    check(ref && ref.url === path.slice('evaluation/evidence-workbench/'.length)
      && Number.isSafeInteger(ref.bytes) && ref.bytes > 0 && ref.bytes <= cap
      && /^[a-f0-9]{64}$/.test(ref.sha256), 'Invalid workbench file reference: ' + path);
    const row = plan.outputs.get(path);
    check(row && row.bytes === ref.bytes && row.sha256 === ref.sha256 && !expected.has(path),
      'Workbench reference differs from public file census: ' + path);
    expected.add(path);
  };
  for (let i = 1; i <= 40; i++) {
    const row = manifest.questions[i - 1], id = `staff-${String(i).padStart(3, '0')}`;
    check(row?.id === id && typeof row.question === 'string' && row.question.length > 0
      && !questions.has(id), 'Invalid workbench question identity');
    questions.set(id, row.question);
    const packagePath = `evaluation/evidence-workbench/packages/${id}.json`;
    publicRef(row.package, packagePath, 524288);
    const parts = row.package.parts;
    check(Array.isArray(parts) && parts.length > 0 && parts.length <= 128, 'Invalid workbench part count');
    for (let j = 0; j < parts.length; j++)
      publicRef(parts[j], `evaluation/evidence-workbench/parts/${id}-${String(j).padStart(3, '0')}.json`, 32768);
  }
  const actual = [...plan.outputs.keys()].filter(path => path.startsWith('evaluation/evidence-workbench/'));
  check(actual.length === expected.size && actual.every(path => expected.has(path)),
    'Public workbench output is not bound to the 40-case manifest');
  return { questions, source: manifest.source };
}
export function validateWorkbenchRegistry(registry, questions) {
  check(Array.isArray(registry?.cases) && registry.cases.length === 40,
    'Committed public question registry has the wrong case count');
  for (let i = 1; i <= 40; i++) {
    const row = registry.cases[i - 1], id = `staff-${String(i).padStart(3, '0')}`;
    check(row?.id === id && row.question === questions.get(id),
      'Workbench questions differ from the committed public registry');
  }
}
export async function admit(repo, site, commit) {
  ...
  const workbenchRaw = await boundedFile(resolve(site, 'evaluation/evidence-workbench/manifest.json'), 262144);
  same(workbenchRaw, plan.outputs.get('evaluation/evidence-workbench/manifest.json'));
  const workbench = validateWorkbenchManifest(strictJson(workbenchRaw), plan);
  for (const [path, row] of plan.outputs) if (path.startsWith('evaluation/evidence-workbench/')) {
    check(!plan.inputs.has(path), 'Workbench input collision'); plan.inputs.set(path, row);
  }
  check(plan.inputs.size <= LIMITS.max_inputs
    && [...plan.inputs.values()].reduce((sum, row) => sum + row.bytes, 0) <= LIMITS.max_input_bytes,
    'Immutable source input bound exceeded');
  ...
  for (const [path, expectedHash] of [[workbench.source.registry, workbench.source.registry_sha256],
    [workbench.source.corpus, workbench.source.corpus_sha256]]) {
    const entry = blobs.get(path);
    check(entry?.[0] === '100644' && entry[1] === 'blob', 'Workbench source is not a regular immutable Git blob');
    const size = Number(execFileSync('git', ['cat-file', '-s', entry[2]], { cwd: repo, maxBuffer: 64 }).toString());
    check(Number.isSafeInteger(size) && size > 0 && size <= 2 * 1024 * 1024, 'Workbench source exceeds byte bound');
    const bytes = execFileSync('git', ['cat-file', 'blob', entry[2]], { cwd: repo, maxBuffer: size + 1024 });
    check(bytes.length === size && digest(bytes) === expectedHash, 'Workbench source hash differs');
    sourceBytes.set(path, bytes);
    gitInputs.push({ path, bytes: size, sha256: expectedHash, git_blob: entry[2] });
  }
  const registry = strictJson(sourceBytes.get(workbench.source.registry));
  validateWorkbenchRegistry(registry, workbench.questions);
...
export async function run({ repo, site, commit, output }, fetcher = fetch) {
  ...
  const receipt = { schema: 'okf-dwp-learning-site-public-verification.v3', ...result, started_at: started,
    ...
    retained_cases: admission?.plan.cases ?? null, workbench_cases: admission?.manifest.evidence_workbench?.questions ?? null,
    git_inputs: admission?.gitInputs ?? [], limits: LIMITS,
    method: 'Immutable Git input checks including all 40 workbench packages and parts, exact local expected outputs, then read-only HTTPS against the fixed Pages host; no retries or outside requests.',
  ...
  const inventory = { schema: 'okf-learning-site-observation-artifacts.v3', files: [...files].map(([path, raw]) => ({ path, bytes: raw.length, sha256: digest(raw) })) };
