...
Status reviewed on 24 September 2026. This is the current short guide to the
...
| --- | --- | --- | --- |
| 1 | Repair the known question failures: capital transfer wording and the new Universal Credit (UC) disabled-child case. Verify the dated legal source before classifying the UC dependency. **BL005, BL006, BL007** | The intended passages and conditional dependencies are selected for fixed positive cases and new wordings, while negative cases stay excluded. Gaps and uncertain applicability remain explicit. | Agent prepares and tests; benefits specialist assesses meaning. |
| 2 | Review the 28 structural outliers before adopting the parked amendment parser. **BL007.remaining-structural-review** | Source reading checks the proposed boundaries, including nine affected substantive chapters. Changed identifiers are reconciled and the full question replay preserves qualifications. | Agent implementation and independent source review. |
...
The first bounded repair is preferable to restarting acquisition or running all
40 questions through more expensive models. The original notional-capital
...
branch remains separate. Documentation and backlog corrections are carried by
[PR 40](https://github.com/chris-page-gov/okf-dwp/pull/40); consult its live checks
before treating it as merged or published.
