...
The [source-led manual guide](docs/source-led-manual-guide.md) explains the next
additive pipeline: understand each manual's local conventions, propose coherent
content units, then use compact discovery cards to find the underlying evidence.
Its source census covers the same 513 captured DMG and ADM documents. PDF pages
remain exact checking locations; headings declared inside a PDF are structural
evidence, not proof of legal meaning. The guide records the candidate's tests,
failed attempts and remaining gaps separately from earlier service releases.

The source-only projection currently contains **53,727 retrievable units** across
19,090 PDF pages, including the 75 earlier author-declared units. The remaining
boundaries are machine proposals. These counts describe source organisation;
they are not counts of complete rules or answerable questions. The separately
authored [selection proposals](domain-profile/structured-units/) add conditional
routes to source passages. The location-migration ledger records how earlier
...
After installing the locked dependencies, these checks replay the retained PDF
structure observations and compare generated outputs. They do not invoke
Poppler, download sources or regenerate the earlier bundle and corpus releases:
...
The [source-led guide](docs/source-led-manual-guide.md) gives the separately
versioned context-comparison procedure and its exact Explorer consumer. A
**discovery card** is a small navigation record pointing to the underlying source
unit; its preview is not evidence. The [PDF observation notes](pdf-structure/README.md)
explain why original trees, failed observations and bounded parser replays are
all retained. Fresh PDF observation is an explicit maintenance operation, outside
normal CI. Missing structure or an unresolved boundary remains visible.

**Continuous integration (CI)** means the automated checks run for each proposed
change. The existing replay and the new source-led checks run in parallel jobs,
each with its own resources. The protected `validate` check succeeds only when
both jobs succeed. The learning website still publishes only after the complete
workflow passes for the exact main-branch commit.
...
- `domain-profile/manual-guide/` authors source-linked manual conventions. `manual-guide/` is its generated guide and document census; each convention has a declared scope.
- `pdf-structure/` retains local observations of source-declared PDF structure, including errors and original producer identities. `pdf-structure/bounded-v2/` is a separately checked offline reparse, not a fresh source capture.
- `structured-units/` and `structured-context/` are additive generated candidates for coherent units, compact discovery cards and bounded context assembly. They leave earlier projections intact and do not establish complete benefits answers.
- `domain-profile/structured-units/` contains explicit source-selection proposals and the policy for navigating from older evidence locations. `structured-context/location-migration.json` records those location mappings, inferred associations and remaining gaps; it does not replace the original evidence requirements.
