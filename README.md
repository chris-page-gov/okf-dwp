...
The [full-DMG acquisition](docs/next-stage/full-dmg-acquisition.md) is complete: **331 PDFs and 14,743 measured pages**, with source hashes and extraction-quality flags. An indexed consumer has passed local source, search and semantic checks; live consumer verification is in progress. Eight wider authoring batches add **245 concepts and 328 proposals** across all 78 substantive PDF units. [Measured coverage](evaluation/full-dmg-coverage.json) distinguishes source-backed proposals from complete policy modelling and specialist acceptance.
