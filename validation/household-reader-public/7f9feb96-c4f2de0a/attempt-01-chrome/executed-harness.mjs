...
async function main() {
  ...
  try {
    ...
    // Reset deliberately removes the exploration toolbar when no reduction or
    // highlight remains. Inspect the Reader's full result count instead.
    await expect(page.locator('.view-heading').filter({ hasText: 'Results' })).toContainText(`${descriptor.counts.records.toLocaleString('en-GB')} matching records`);
