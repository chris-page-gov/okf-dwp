...
DEST = ROOT / 'source/legal-bodies-2026-09-21'
...
def project_xml(data, seed, requested_date):
    ...
    for unit in seed['units']:
        ...
        result['units'].append({'target': unit, 'canonical_identifier': identifier,
            ...
            'commentary_ids': sorted({e.get('Ref') for e in selected.iter() if local(e.tag) == 'CommentaryRef' and e.get('Ref')})})
    if len(refs) > 200: raise ValueError('Commentary count exceeds budget')
    for ref in sorted(refs):
        if ref not in commentaries: result['missing_commentary_ids'].append(ref); continue
        tree = safe_tree(commentaries[ref], result['omitted_attributes'], 'Commentary/' + ref)
        result['selected_commentaries'].append({'id': ref, 'type': commentaries[ref].get('Type'), 'tree': tree, 'text': body_text(tree)})
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        for name, row in pool.map(acquire, config['requests']):
    ...
    files['request-census.json'] = pretty({'schema': 'okf-legal-body-request-census.v1', 'requests': receipts, 'count': len(receipts),
        'preflight_note': config['preflight_note'], 'automatic_retries': 0, 'raw_response_bodies_retained_publicly': False})
