...
def run_one(provider, case, manifest, protocol, attempt, run):
    ...
    if receipt_path.exists():
        ...
        if receipt["inputs"] != binding:
            raise ValueError("Existing attempt has different fixed input; preserve it and use a new attempt")
    ...
    receipt = {"schema": "okf-paired-staff-model-attempt.v1", "provider": provider, "case_id": case["id"], "attempt": attempt,
               "started_at": started, "inputs": binding, "harness_sha256": sha(Path(__file__).read_bytes()),
    ...
    try:
        ...
        elif model_output.get("tool_event_names"):
