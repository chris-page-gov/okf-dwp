...
assert(!existsSync(out),'Retain earlier attempts; choose a fresh name');
...
writeFileSync(resolve(out,'evaluator.mjs'),readFileSync(fileURLToPath(import.meta.url)));
...
for(const test of cases)for(const run of sources)for(const max_bytes of protocol.budgets){
 ...
 const archive=`${run.stage}-${test.id}-${max_bytes}.json.gz`;writeFileSync(resolve(out,archive),gzipSync(bytes,{level:9,mtime:0}));
 ...
 writeFileSync(resolve(out,'progress.json'),JSON.stringify({completed:rows.length,last:{case_id:test.id,stage:run.stage,max_bytes}},null,2)+'\n');
}
...
writeFileSync(resolve(out,'timing.json'),JSON.stringify({observed_at:new Date().toISOString(),rows:timing},null,2)+'\n');
console.log(JSON.stringify({assemblies:rows.length,staff_occurrences:40,all_insufficient:rows.every(r=>r.evidence_status==='insufficient'),network_calls:0,model_calls:0}));
 ...
 writeFileSync(resolve(out,'failure.json'),JSON.stringify({status:'failed',current,error:String(error),stack:String(error.stack||'').slice(0,8000),
