...
for (const test of cases) for (const run of sources) for (const max_bytes of [32768, 524288]) {
  ...
  for (const {record} of evidence) for (const p of record.provenance) if (p.literal_sha256) assert.equal(sha(record.text), p.literal_sha256);
...
if (check) {
  ...
  for (const [name, raw] of packages) assert.deepEqual(gunzipSync(read(resolve(out, name))), gunzipSync(raw));
} else {
