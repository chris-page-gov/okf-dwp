---
'@id': https://chris-page-gov.github.io/okf-dwp/id/story/evidence-audit
route: story/evidence-audit
title: Audit a source transformation
type: UserStory
description: As a knowledge engineer, I want an extract, original PDF hash and relationship
  evidence, so that I can reproduce and inspect the transformation.
status: draft
tags:
- unofficial
- project-authored
- userstory
id: story/evidence-audit
'@type': schema:CreativeWork
generated:
  by: process:codex-research-authoring
  at: '2026-09-15T14:21:03Z'
authority: model-assisted research navigation; unreviewed
dcterms:references:
- '@id': https://chris-page-gov.github.io/okf-dwp/id/persona/knowledge-engineer
- '@id': https://chris-page-gov.github.io/okf-dwp/id/term/capital-disregards
---

# Audit a source transformation

Independent project-authored discovery aid; not an official interpretation or entitlement decision.

As a knowledge engineer, I want an extract, original PDF hash and relationship evidence, so that I can reproduce and inspect the transformation.

[Research reference: Knowledge engineer](https://github.com/chris-page-gov/okf-dwp/blob/main/bundle/records/persona/knowledge-engineer.md).

[Research reference: Capital disregards](https://github.com/chris-page-gov/okf-dwp/blob/main/bundle/records/term/capital-disregards.md).
